...
//FIXME: is the index necessary to delete a key from an array?
...
module.exports = {
  ...
  JsonBucket: function(file) {
    ...
    this.incrementKey = function() {
      //FIXME: not positive lastkey makes it into this scope nor the setter for the key
      //FIXME: clean this up, this is for debugging
      console.log("INCREMENT KEY CALLED. Key currently " + this.key);
      var ret = ++this.key;
      console.log("RETURNING " + ret + " KEY ON RECORD IS NOW " + this.key);
      return ret;
      //return ++this.key;
    };
    ...
    // Sync
    this.read = function() {
      this.dataset = fs.readFileSync(this.file, function(err, data) {
        if (err) throw err;
        //WHAT IS THIS?
    ...
    // Async
    this.flush = function() {
      console.log("FLUSHING FILE " + this.file);
